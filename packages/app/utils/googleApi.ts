...
function format(raw: any) {
  ...
  if (raw.geometry && raw.geometry.location) {
    address.position = {
      latitude: raw.geometry.location.latitude,
      longitude: raw.geometry.location.longitude
    };
  }
...
export default {
  geocodePosition(apiKey: string, position: any, language: LanguageOption) {
    ...
    return this.geocodeRequest(
      `${googleUrl}?key=${apiKey}&language=${language}&latlng=${
        position.latitude
      },${position.longitude}`
    );
  },

  geocodeAddress(apiKey: string, address: string, language: LanguageOption) {
    if (!apiKey || !address) {
      return Promise.reject(new Error('invalid apiKey / address'));
    }

    return this.geocodeRequest(
      `${googleUrl}?key=${apiKey}&address=${encodeURI(
        address
      )}&language=${language}`
    );
  },
  ...
  async geocodeRequest(url: string) {
    ...
    if (!json.results || json.status !== 'OK') {
      return Promise.reject(
        new Error(`geocoding error ${json.status}, ${json.error_message}`)
      );
    }
