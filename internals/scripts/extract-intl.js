...
const { appLocales, DEFAULT_LOCALE } = require('../../packages/app/i18n');

const babel = require('../../translate.babel.config.js');
const { presets } = babel;
...
const FILES_TO_PARSE = 'packages/app/**/messages.ts';
...
for (const locale of appLocales) {
  ...
  const translationFileName = `packages/app/translations/${locale}.json`;
  try {
...
memoryTask.then(files => {
  ...
  extractTask.then(() => {
    ...
    mkdir('-p', 'packages/app/translations'); // eslint-disable-line

    let localeTaskDone;
    let translationFileName;

    for (const locale of appLocales) {
      translationFileName = `packages/app/translations/${locale}.json`;
      localeTaskDone = task(
